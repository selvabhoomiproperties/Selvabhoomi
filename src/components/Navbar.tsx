    ...
    return (
        <nav className={`fixed w-full z-50 transition-all duration-500 ${scrolled ? 'py-4' : 'py-8'}`}>
            <div className="max-w-7xl mx-auto px-6 md:px-8 lg:px-10">
                <div className={`relative overflow-hidden rounded-2xl transition-all duration-500 ${scrolled ? 'bg-black/60 backdrop-blur-2xl border border-white/10 shadow-2xl px-6 md:px-8 py-4' : 'px-4 md:px-6 py-2'}`}>
                    <div className="flex items-center justify-between">
                        <Link to="/" className="flex items-center gap-3 group relative z-10 transition-transform hover:scale-105 active:scale-95">
                            <motion.div
                            ...
                            <EditableText
                                ...
                                content="SELVABHOOMI"
                                as="span"
                                className={`text-xl md:text-2xl font-black tracking-tighter transition-colors duration-300 ${scrolled ? 'text-white' : 'text-black'}`}
                            />
                        </Link>
                        ...
                        {/* Mobile Menu Button */}
                        <button
                            className="md:hidden relative z-10 p-3 text-gray-400 hover:text-white bg-white/5 rounded-xl border border-white/10"
